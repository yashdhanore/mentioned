...
def upgrade() -> None:
    ...
    op.execute(
        "REVOKE ALL ON TABLE public.sources, public.source_items, public.saved_sources FROM anon, authenticated"
    )
    ...
    op.execute(
        "CREATE POLICY sources_api_insert ON public.sources FOR INSERT TO mentioned_api WITH CHECK (true)"
    )
    op.execute(
        "CREATE POLICY sources_api_retry_update ON public.sources FOR UPDATE TO mentioned_api USING (true) WITH CHECK (true)"
    )
    op.execute(
        "CREATE POLICY source_items_api_select ON public.source_items FOR SELECT TO mentioned_api USING (true)"
    )
    op.execute(
        """
    ...
    op.execute(
        "CREATE POLICY sources_worker_all ON public.sources FOR ALL TO mentioned_worker USING (true) WITH CHECK (true)"
    )
    op.execute(
        "CREATE POLICY source_items_worker_all ON public.source_items FOR ALL TO mentioned_worker USING (true) WITH CHECK (true)"
    )
    op.execute(
        "CREATE POLICY saved_sources_worker_all ON public.saved_sources FOR ALL TO mentioned_worker USING (true) WITH CHECK (true)"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "REVOKE SELECT, UPDATE, DELETE ON TABLE pgmq.q_extract_sources FROM mentioned_worker"
    )
    ...
    op.execute(
        "REVOKE ALL ON TABLE public.sources, public.source_items, public.saved_sources FROM mentioned_api, mentioned_worker"
    )
