...
- E2E tests live in `tests/e2e/`, run against the real local stack from `make dev` (Supabase Auth, Postgres with the `mentioned_api` role, API processes they boot themselves), and skip when that stack is not running.
  ...
  `FakeLangfuse` decodes the Langfuse SDK's real OTLP exports and score batches; `tests/e2e/test_extraction_tracing.py` points tracing at it.
  `tests/conftest.py` sets `LANGFUSE_TRACING_ENABLED=false` so no test reaches Langfuse Cloud; a test that traces sets it back to `true` and configures `src.observability` with a public key of its own, because the SDK keeps one client per key for the life of the process.
